...
const REQUIRED_BINS = [
  ...
  // tmux keeps terminal/agent sessions alive while the Node hub is restarted.
  ['tmux', 'tmux'],
...
async function tick() {
  ...
  try {
    ...
    // Экран не должен гаснуть при бездействии: звук (pulse) играет, а
    // изображение уходит в черноту из-за X screensaver/DPMS — на телефоне это
    // выглядит как «видео играет, экрана нет». Отключаем засыпание на каждом
    // тике, чтобы ремонт/перезапуск X его не вернули.
    if (x) await sh(`DISPLAY=${DISPLAY} xset s off -dpms s noblank 2>/dev/null || true`, 3000);
