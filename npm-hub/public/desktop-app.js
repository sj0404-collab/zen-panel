...
async function createTerm(toolId, cwdOverride, plainTerminal, resumeSession) {
  ...
  panel.id = 'panel-' + id;
  panel.innerHTML = `<div class="term-header"><div class="term-header-title"><div style="width:8px;height:8px;border-radius:50%;background:${color}"></div>${displayName}</div><div class="term-info">${dirShort}</div><button class="btn btn-sm" onclick="closeTab('${id}')">✕</button></div><div class="term-wrap"><div class="term" id="term-${id}"></div><div class="term-scroll" id="tscroll-${id}"><div class="term-mouse-pad" title="Виртуальная мышь: тянуть — прокрутка, тап сверху/снизу — шаг">🖱</div><div class="term-mouse-wheel"><button class="term-scroll-arr up">▲</button><button class="term-scroll-arr down">▼</button></div><button class="term-scroll-handle" title="Потянуть — переместить мышь; тап — вернуть наверх">⠿</button></div></div><div class="term-cd" id="cd-${id}"></div>`;
