        ...
        ${(rel.assets || []).map(a => `
          <div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:11px">
            ...
            <div style="margin-left:auto;display:flex;gap:4px">
              <a href="/api/gh/download-release-asset?full_name=${encodeURIComponent(ghCurrentRepo)}&tag=${encodeURIComponent(rel.tag_name)}&asset_id=${a.id}" download class="btn btn-sm" style="text-decoration:none;font-size:10px;color:var(--ok)" title="Скачать через сайт (как кнопка сборки)">📥 Через сайт</a>
