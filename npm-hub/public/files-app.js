...
async function fmViewOpen(p, name, kind) {
  ...
  if (kind === 'text' && blob.size <= FM_TEXT_MAX) addBtn('💾 Сохранить', 'btn-p', 'fm-view-save', () => fmViewSave(p));
  addBtn('⬇ Скачать', '', '', () => { fmSaveBlob(blob, name); });
  ...
  if (kind === 'image' || kind === 'video' || kind === 'audio' || kind === 'pdf') {
    ...
    ta.id = 'fm-view-text';
    ta.spellcheck = false;
    const tooBig = blob.size > FM_TEXT_MAX;
    ta.readOnly = tooBig;
