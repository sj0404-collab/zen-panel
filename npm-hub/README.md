...
1. Предустановка CLI по умолчанию выключена (`preinstall: false`):
   хаб встаёт за секунды, а каждый инструмент докачивается при первом
   клике — и только тот, что реально нужен. Кэш npm/pip переживает
   раны (накапливающийся `actions/cache`), так что повторные запуски
   почти ничего не качают. Для полной предустановки — `preinstall: true`
   (один вызов `npm` + поштучный fallback + `agent` на PATH — он ставится
   всегда).
...
- **Инструменты** (`/api/tools`): автоопределение установленных CLI
  ...
  Инструменты с npm-пакетом ставятся при первом клике (`npx -y`), если
  их нет; `agent` (zen-agent) на раннерах всегда кладётся на PATH.
  Пятёрка без npm-пакета (`ccb`, `koda`, `openclaude`, `openrouter`,
  `crush`) работает только там, где стоит локально: в npm под этими
  именами лежат чужие пакеты.
...
Локально доступа по паролю нет: сервер слушает `0.0.0.0` ради телефона
в LAN, любой в сети получает shell. Не публикуйте URL туннеля, не
запускайте в недоверенных сетях. Весь shell-стринг с пользовательским
вводом (ADB/FTP) санитизируется, но модель угроз — «доверенная локалка».

Запуски из панели (`hub.yml`) закрыты гейт-токеном: панель генерирует
случайный `token` на запуск, передаёт его workflow-input'ом (в репозиторий
